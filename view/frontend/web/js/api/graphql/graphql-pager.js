define([], function () {
    ...
    return function (known, toLoad) {
        ...
        if (known < 0 || toLoad <= 0) {
            return {pageSize: 0, currentPage: 1, drop: 0}
        }
        if (toLoad > known) {
            return {pageSize: known + toLoad, currentPage: 1, drop: known}
        }
        const pageSize = findNextDivisor(known, toLoad);
        return {pageSize: pageSize, currentPage: (known / pageSize) + 1, drop: 0}
    }
});
